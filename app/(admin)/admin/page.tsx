...
export default function AdminPage() {
  ...
  const getUserList = async () => {
    const response = await fetch('https://192.168.31.16:8000/users')
    const userList: User[] = await response.json()
    setUserList(userList)
  }

  const getSumConversationsAndMessages = async () => {
    const response = await fetch('https://192.168.31.16:8000/sum-conversations-and-messages')
  ...
  const editUser = async (data: Partial<UserFormFields>) => {
    ...
    const response = await fetch(`https://192.168.31.16:8000/users/${selection[0]}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
